...
export default async function handler(
  ...
  try {
    ...
    const response = await axios.get('https://xskt.net/');
    ...
    // Get rows
    bodyElement.find('table.table-lotto').find('tr:not(:first-child)').each((index, row) => {
      if (index > 10) return
      const rowData: any = {};
      $(row).find('.txt-normal-prize, .txt-special-prize').each((index, cell) => {
        rowData[headers[index] || `column${index}`] = $(cell).text().trim();
      });
